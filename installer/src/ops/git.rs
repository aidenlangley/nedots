...
#[derive(Debug, Error)]
/// Errors returned during `GitOp` runtime.
pub(crate) enum GitError {
    #[error("No path to a `git` repository was provided.")]
    /// Thrown by a poorly constructed `GitOp`.
    NoPath,
...
/// A `GitOp` is constructed and passed a `path`, then a `Repository` is
/// opened and referenced throughout its' lifetime.
pub(crate) struct GitOp<'remote> {
    ...
    /// Remote to push/pull from.
    remote: Option<&'remote str>,
}

impl<'remote> GitOp<'remote> {
    /// Construct a new `GitOp`.
    ...
    pub(crate) fn at_path(mut self, path: &Path) -> Result<Self, GitError> {
    ...
    /// Assign `remote`.
    pub(crate) fn with_remote(mut self, remote: &'remote str) -> Self {
        self.remote = Some(remote);
        self
    }

    /// Get the `path` of git `Repository`.
    pub(crate) fn path(&self) -> Result<&Path, GitError> {
        self.path
            .as_ref()
            .map(|pb| pb.as_path())
    ...
    pub(crate) fn repo(&mut self) -> Result<&Repository, GitError> {
        Ok(self.repo.get_or_insert(Repository::open(self.path()?)?))
    }

    /// Adds latest changes from a dirty tree.
    pub(crate) fn add_changes(mut self) -> Result<(Self, Oid), GitError> {
    ...
    pub(crate) fn commit(mut self, tree_id: Oid) -> Result<(Self, Oid), GitError> {
        let repo = self.repo()?;
    ...
    pub(crate) fn push(&mut self, remote: &str) -> Result<(), GitError> {
        let repo = self.repo()?;
    ...
    pub(crate) fn fetch(
        ...
        branch: &str,
        remote: &str,
    ) -> Result<AnnotatedCommit, GitError> {
        let repo = self.repo()?;
    ...
    pub(crate) fn rebase(&mut self) -> Result<(), GitError> {
        todo!()
    }

    pub(crate) fn merge(&mut self) -> Result<(), GitError> {
        todo!()
    }

    pub(crate) fn stash(&mut self) -> Result<Oid, GitError> {
        todo!()
    }

    pub(crate) fn restore(mut self) -> Result<Self, GitError> {
        todo!()
    }
}
...
mod tests {
    ...
    #[test]
    /// Expects to fail because a `GitOp` was made without a `path` to a
    /// `Repository`.
    fn no_path() {
        if let Err(e) = GitOp::new().path() {
            assert_eq!(e.to_string(), "No path to a `git` repository was provided.")
        }
    }

    #[test]
    /// Expects to fail because a `GitOp` was made with a `path` to a
    /// directory that isn't a git `Repository`.
    fn bad_path() {
        if let Err(e) = GitOp::new().at_path(Path::new(_TESTS_DIR)) {
            assert!(e
                .to_string()
    ...
    /// (without leaning on `GitOp`) to make the initial commit.
    ///
    /// Once the initial commit is made, dirty the tree again via `add_changes`,
    /// the runs `commit` via `GitOp` to test its impl.
    ///
    ...
    fn add_commit() {
        ...
        // Make GitOp & open repo.
        let go = GitOp::new()
            .at_path(&path)
            .expect("Failed to create `GitOp`!");
        ...
        // Commit the changes, this time via `GitOp::commit`. Again, mutated so re-assigned.
